...
function autoReservation(availableOption, htlmObj) {
  ...
  for (let idx = 0; idx < availableOption.length; idx++) {
    link = htlmObj(`a div h4:contains(${availableOption[idx]})`).parent().parent().attr('href')
    if (link) break
  }
  if (link) {
    rp(baseurl + link).then(html => {
      ...
      let buyticketLink = ''
      for (let index = 0; index < ticket.sort(preferNumberTickets).length; index++) {
    ...
    rp(url)
      ...
      .then(html => {
        ...
        const data = $('span').filter(function () {
          return $(this).text().trim() === reactiveValLabel;
        }).parent().parent()
        ...
        if (info.dispo) {
          ...
          const optionsAvailable = $('a h4').parent().has('strong').map(function () {
            return $(this).find('h4').text().trim()
          }).get()

          if (optionsAvailable.some(opt => interestedOptions.includes(opt))) {
            console.log(chalk.yellow(time), separator, chalk.green.inverse(info.info.map(el => `${el.label}: ${el.val}`).join(separator)))
            ...
            if (!autoReservationBool) {
              ...
              autoReservation(optionsAvailable.filter(opt => interestedOptions.includes(opt)), $)
            }
          } else {
            console.log(chalk.yellow(time), separator, chalk.green(info.info.map(el => `${el.label}: ${el.val}`).join(separator)))
            console.log(chalk.bgMagenta(url))
          }
        } else {
          //screen time and info in color depending if ticket are available or not
          console.log(chalk.yellow(time), separator, chalk.blue(info.info.map(el => `${el.label}: ${el.val}`).join(separator)))
        }
      })
